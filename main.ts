...
const getBrands = async () => {
    ...
    const tokenData = await getAuthToken().then(r => r) as {access_token: string}
...
const uploadFile = async () => {
    ...
    const jobName = await job.getName(true) as string;
    ...
    if (debugLog == 'Yes') await job.log(LogLevel.Debug, 'Hema-New' + "formData: %1", [formData.entries.name])


    const brands = await getBrands().then(r => r) as {brandId: string}[]
    const body = new URLSearchParams({
    client_id: 'preprod-package-specs',
    client_secret: 'hcs_Q8j1kUjpPZlmX686mO0ZG8p1veUAcfprznOF',
    grant_type: 'client_credentials',
...
try {
    if (brands && brands.length > 0) {
        const brand = brands?.find(brand => brand) as {brandId: string}

        const response = await fetch(`${URL}${brand.brandId}`, options)
